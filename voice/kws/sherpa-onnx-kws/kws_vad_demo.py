...
def main():
    ...
    vad = sherpa_onnx.VoiceActivityDetector(create_vad_config(args), buffer_size_in_seconds=60)

    kws_stream = kws.create_stream()

    if args.wav:
        wav_file = wave.open(args.wav)
        if wav_file.getnchannels() != 1 or wav_file.getsampwidth() != 2 or wav_file.getframerate() != args.sample_rate:
    ...
    def finalize_command():
        ...
        if not vad.empty():
            ...
            if len(segment) > 0:
                ...
                print(f"[VAD] 命令音频已保存: {output_path} ({duration:.2f}s) [覆盖更新]\n")
            else:
