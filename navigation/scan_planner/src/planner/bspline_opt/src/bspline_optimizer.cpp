...
namespace scan_planner
{
  ...
  void BsplineOptimizer::setParam(rclcpp::Node *node)
  {
    ...
    /* height-adaptive parameters (HAVEN) */
    height_min_ = get_double("optimization.height_min", 0.17);
    height_max_ = get_double("optimization.height_max", 0.40);
    height_nominal_ = get_double("optimization.height_nominal", 0.28);
    ceiling_margin_ = get_double("optimization.ceiling_margin", 0.03);
    body_top_offset_ = get_double("optimization.body_top_offset", 0.328);
    lambda_ceiling_ = get_double("optimization.lambda_ceiling", 100.0);
    lambda_height_reg_ = get_double("optimization.lambda_height_reg", 0.5);
    lambda_height_lim_ = get_double("optimization.lambda_height_lim", 200.0);
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    for (int i = order_; i <= i_end; ++i)
    {
      for (double a = 1.0; a >= 0.0; a -= step_size)
      {
        ...
        occ = grid_map_->getInflateOccupancyAdaptive(sample_pt, sample_yaw);
        // cout << setprecision(5);
  ...
  /***
   * 高度自适应代价 (HAVEN): 只作用于控制点 z 行
   * z 为 base_link 世界高度, 机器人顶部 = z + body_top_offset_
   *
   * 1) 天花板碰撞: 机器人顶部 + m_v 超过足迹内最低天花板时, 用与
   *    距离代价相同的三次平滑惩罚把机体压下去;
   * 2) 腿高限位: 超出五连杆物理范围 [h_min, h_max] 的二次惩罚;
   * 3) 高度正则: 弱拉回标称高度 h_star, 开阔区域保持站立高度。
   * 返回的 cost/gradient 已各自乘上对应 lambda。
   */
  void BsplineOptimizer::calcHeightCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    cost = 0.0;

    const int end_idx = q.cols() - order_;
    const double demarcation = cps_.clearance;
    const double a3 = 3 * demarcation, b3 = -3 * pow(demarcation, 2), c3 = pow(demarcation, 3);
    ...
    for (int i = order_; i < end_idx; ++i)
    {
      ...
      /* 天花板碰撞惩罚 */
      const double yaw = estimateControlPointYaw(q, i);
      const double z_ceil = grid_map_->queryCeilingHeight(q.col(i), yaw);
      if (std::isfinite(z_ceil))
      {
        const double err = z + body_top_offset_ + ceiling_margin_ - z_ceil;
        if (err > 0.0)
        {
          if (err < demarcation)
          {
            cost += lambda_ceiling_ * pow(err, 3);
            gradient(2, i) += lambda_ceiling_ * 3.0 * err * err;
          }
          else
          {
            cost += lambda_ceiling_ * (a3 * err * err + b3 * err + c3);
            gradient(2, i) += lambda_ceiling_ * (2.0 * a3 * err + b3);
          }
        }
      }
      ...
      if (z < height_min_)
      {
        const double d = height_min_ - z;
        cost += lambda_height_lim_ * d * d;
        gradient(2, i) += -2.0 * lambda_height_lim_ * d;
      }
      else if (z > height_max_)
      {
        const double d = z - height_max_;
        cost += lambda_height_lim_ * d * d;
        gradient(2, i) += 2.0 * lambda_height_lim_ * d;
      }

      /* 高度正则: 拉回标称腿高 */
      const double dr = z - height_nominal_;
      cost += lambda_height_reg_ * dr * dr;
      gradient(2, i) += 2.0 * lambda_height_reg_ * dr;
    }
  }
  ...
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      bool occ = grid_map_->getInflateOccupancyAdaptive(cps_.points.col(i), estimateControlPointYaw(cps_.points, i));
      ...
      if (occ)
      {
        ...
        for (j = i - 1; j >= 0; --j)
        {
          occ = grid_map_->getInflateOccupancyAdaptive(cps_.points.col(j), estimateControlPointYaw(cps_.points, j));
          if (!occ)
          {
            in_id = j;
            break;
          ...
          if (got_intersection_id >= 0)
          {
            ...
            if (length > 1e-5)
            {
              for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
              {
                Eigen::Vector3d sample_pt = (a / length) * intersection_point + (1 - a / length) * cps_.points.col(j);
                double sample_yaw = estimateControlPointYaw(cps_.points, j);
                bool occ = grid_map_->getInflateOccupancyAdaptive(sample_pt, sample_yaw);
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        // 高度限位是机构硬约束，不能仅依赖软代价；钳制后再做碰撞检查。
        for (int i = start_id; i < end_id; ++i)
          cps_.points(2, i) = std::clamp(cps_.points(2, i), height_min_, height_max_);
        ...
        for (double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
          ...
          flag_occ = grid_map_->getInflateOccupancyAdaptive(pos, estimateSegmentYaw(pos, pos_next));
          if (flag_occ)
          {
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      for (double t = tm; t < tmp * 2 / 3; t += t_step)
      {
        Eigen::Vector3d pos = traj.evaluateDeBoorT(t);
        Eigen::Vector3d pos_next = traj.evaluateDeBoorT(std::min(t + t_step, tmp));
        if (grid_map_->getInflateOccupancyAdaptive(pos, estimateSegmentYaw(pos, pos_next)))
        {
  ...
  void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    double f_smoothness, f_distance, f_feasibility, f_height;
    ...
    f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + f_height;
    //printf("origin %f %f %f %f\n", f_smoothness, f_distance, f_feasibility, f_combine);

    Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + new_lambda2_ * g_distance +
                              lambda3_ * g_feasibility;
    // 侧向代价不驱动高度; z 方向由平滑/可行性/高度代价接管 (HAVEN)
    grad_3D.row(2) = lambda1_ * g_smoothness.row(2) + lambda3_ * g_feasibility.row(2) +
                     g_height.row(2);
  ...
  void BsplineOptimizer::combineCostRefine(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    /* ---------- evaluate cost and gradient ---------- */
    double f_smoothness, f_fitness, f_feasibility, f_height;
    ...
    f_combine = lambda1_ * f_smoothness + lambda4_ * f_fitness + lambda3_ * f_feasibility + f_height;
