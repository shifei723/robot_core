...
namespace scan_planner
{
  ...
  private:
    ...
    /* height-adaptive parameters (HAVEN), z = base_link world height */
    double height_min_, height_max_, height_nominal_; // body height range and nominal
    double ceiling_margin_;                           // vertical clearance to ceiling
    double body_top_offset_;                          // base_link -> robot top (lidar)
    double lambda_ceiling_;                           // ceiling collision weight
    double lambda_height_reg_;                        // pull back to nominal height
    double lambda_height_lim_;                        // hard range enforcement
    bool allow_unverified_ceiling_clearance_;         // static simulation clearance policy
