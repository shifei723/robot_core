...
struct MappingParameters {
  ...
  /* 动态拖影衰减: 本帧未被真实命中的占位 cell 每帧递减的 log-odds
     用于快速释放动态障碍物离开后的旧占位。<=0 为关闭。 */
  double decay_log_;

  /* 高度自适应 (HAVEN): base_link 世界高度范围与天花板安全余量
     body_top_offset_: base_link 到机器人最高点(雷达顶)的距离,
     机器人包络顶部 = base_z + body_top_offset_ */
  double height_min_pass_, height_max_, ceiling_margin_, body_top_offset_;
  // 仅静态仿真可开启：允许用已识别的天花板净空穿越下方未知体素。
  bool allow_unverified_ceiling_clearance_;

  /* 世界边界: 超出该范围的位置直接视为占据, 保证边界外不可达 */
  bool world_bound_en_;
  Eigen::Vector2d world_bound_min_, world_bound_max_;
...
struct MappingData {
  ...
  vector<char> flag_traverse_, flag_rayend_, flag_hit_;
  char raycast_num_;
  queue<Eigen::Vector3i> cache_voxel_;

  // 占位 cell 地址表 (供阴影区衰减遍历, 避开全图扫描), 惰性删除

  vector<int> occ_cells_;
  vector<char> in_occ_list_;

  // 天花板查询足迹偏移 (单圆柱圆盘, 米), 查询时旋转 yaw 并平移前后双圆心

  vector<Eigen::Vector2d> ceiling_offsets_;
...
public:
  ...
  inline int getInflateOccupancyAdaptive(const Eigen::Vector3d& pos, double yaw);
  inline bool isInWorldBound(const Eigen::Vector3d& pos) const;
  /**
   * 查询足迹内最低天花板高度。
   * 返回值: 已观测到的最低占据体素底面 z; 未发现则为 +inf。
   * clearance_verified: 天花板下方是否已确认为自由空间
   *   (遇到未知体素或足迹超图时为 false)。
   * 注: 未知不会把返回值拉到 0, 否则代价函数会误以为
   *     “天花板就在地面”而让机器人全程下蹲。
   */
  double queryCeilingHeight(const Eigen::Vector3d& pos, double yaw,
                            bool* clearance_verified = nullptr);
...
/**
 * 高度自适应通行性查询 (HAVEN 指南第 9 节的最小实现)
 *
 * 膨胀层把悬空横梁向下膨胀到地面, 会堵死本可下蹲通过的走廊。
 * 此处先查足迹内的真实天花板高度, 再按当前 base_link 高度判断净空。
 * 因此三维 A* 可以在横梁前下蹲、通过后起身, 轨迹碰撞检查也不会把
 * 站立高度误判为可通过。默认情况下，天花板下方存在未知体素会回退到
 * 原始膨胀查询；仅静态仿真可显式放宽该限制。
 */
inline int GridMap::getInflateOccupancyAdaptive(const Eigen::Vector3d& pos, double yaw) {
  // 边界外一律不可达
  if (mp_.world_bound_en_ && !isInWorldBound(pos)) return 1;

  if (!isInMap(pos)) return -1;

  // 轨迹 z 是 base_link 世界高度，必须位于五连杆允许的高度范围内。
  // 同时约束三维 A* 的状态层数，避免从横梁下方无意义地向高空扩展。
  if (pos(2) < mp_.height_min_pass_ || pos(2) > mp_.height_max_) return 1;

  bool verified = false;
  const double z_ceil = queryCeilingHeight(pos, yaw, &verified);
  if ((verified || mp_.allow_unverified_ceiling_clearance_) && std::isfinite(z_ceil)) {
    const double max_base_z = z_ceil - mp_.body_top_offset_ - mp_.ceiling_margin_;
    return pos(2) <= max_base_z ? 0 : 1;
  }

  return getInflateOccupancy(pos, yaw);
}

inline bool GridMap::isInWorldBound(const Eigen::Vector3d& pos) const {
  return pos(0) >= mp_.world_bound_min_(0) && pos(0) <= mp_.world_bound_max_(0) &&
         pos(1) >= mp_.world_bound_min_(1) && pos(1) <= mp_.world_bound_max_(1);
}
