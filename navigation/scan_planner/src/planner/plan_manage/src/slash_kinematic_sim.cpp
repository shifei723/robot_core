...
{
class SlashKinematicSim : public rclcpp::Node
{
public:
  SlashKinematicSim() : Node("slash_kinematic_sim")
  {
    x_ = declare_parameter<double>("init_x", 0.0);
    y_ = declare_parameter<double>("init_y", 0.0);
    leg_height_min_ = declare_parameter<double>("leg_height_min", 0.13);
    leg_height_max_ = declare_parameter<double>("leg_height_max", 0.37);
    // base_link 世界高度 = 腿高 + 偏置 (轮半径 0.0922 - 高电机高 0.06)
    // 保证任意腿高下轮子都刚好贴地
    base_z_offset_ = declare_parameter<double>("base_z_offset", 0.0322);
    // init_z 为 base_link 世界高度, 反推初始腿高
    leg_h_ = std::clamp(declare_parameter<double>("init_z", 0.28) - base_z_offset_,
                        leg_height_min_, leg_height_max_);
    target_leg_h_ = leg_h_;
    z_ = leg_h_ + base_z_offset_;
    ...
    leg_height_rate_ = declare_parameter<double>("leg_height_rate", 0.3);
    ...
    cmd_sub_ = create_subscription<geometry_msgs::msg::Twist>(
        "cmd_vel", 20, std::bind(&SlashKinematicSim::cmdCallback, this, std::placeholders::_1));
    height_sub_ = create_subscription<std_msgs::msg::Float32>(
        "leg_height_cmd", 10,
        std::bind(&SlashKinematicSim::heightCallback, this, std::placeholders::_1));
    last_cmd_time_ = now();
    last_sim_time_ = now();
    timer_ = create_wall_timer(
        std::chrono::duration<double>(1.0 / std::max(1.0, sim_rate)),
        std::bind(&SlashKinematicSim::simCallback, this));
    RCLCPP_INFO(get_logger(), "Slash kinematic simulator ready (child_frame_id=%s)", child_frame_id_.c_str());
  }
...
private:
  ...
  void heightCallback(const std_msgs::msg::Float32::ConstSharedPtr msg)
  {
    target_leg_h_ = std::clamp(static_cast<double>(msg->data), leg_height_min_, leg_height_max_);
  }
  ...
  void simCallback()
  {
    ...
    // 腿高以斜坡速率跟随指令; base_link 高度始终比腿高高一个固定偏置,
    // 使轮子在任意腿高下都恰好贴地
    const double dh = std::clamp(target_leg_h_ - leg_h_, -leg_height_rate_ * dt, leg_height_rate_ * dt);
    leg_h_ += dh;
    z_ = leg_h_ + base_z_offset_;
  ...
  double x_{0.0}, y_{0.0}, z_{0.28}, yaw_{0.0};
  double leg_h_{0.25}, target_leg_h_{0.25};
  ...
  double leg_height_min_{0.13}, leg_height_max_{0.37}, leg_height_rate_{0.3};
  double base_z_offset_{0.0322};
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<scan_planner::SlashKinematicSim>());
  rclcpp::shutdown();
  return 0;
}