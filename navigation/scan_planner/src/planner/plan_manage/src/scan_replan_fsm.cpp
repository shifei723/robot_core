...
namespace scan_planner
{
  ...
  bool SCANReplanFSM::adjustGlobalTargetIfOccupied()
  {
    ...
    const int final_occ = map->getInflateOccupancyAdaptive(
        final_pt, estimateYawFromSegment(final_prev, final_pt));
    if (final_occ <= 0)
      return true;
    ...
    for (int i = sample_num; i >= 0; --i)
    {
      ...
      if (map->getInflateOccupancyAdaptive(pt, estimateYawFromSegment(prev_pt, pt)) == 0)
      {
  ...
  void SCANReplanFSM::execFSMCallback()
  {
    ...
    case REPLAN_TRAJ:
    {
      // 轨迹已把机器人带到目标附近时，不应再把 reboundReplan 的
      // “Close to goal” 当作失败反复重试。
  ...
  void SCANReplanFSM::checkCollisionCallback()
  {
    ...
    for (double t = t_cur; t < info->duration_; t += time_step)
    {
      ...
      if (map->getInflateOccupancyAdaptive(pos, estimateYawFromSegment(pos, pos_next)))
      {
  ...
  void SCANReplanFSM::getLocalTarget()
  {
    ...
    auto targetOccupancy = [&](const Eigen::Vector3d &pt) {
      return planner_manager_->grid_map_->getInflateOccupancyAdaptive(
          pt, estimateYawFromSegment(odom_pos_, pt));
    };
