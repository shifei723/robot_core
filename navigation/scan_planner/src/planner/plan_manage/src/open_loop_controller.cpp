...
public:
  OpenLoopController() : Node("open_loop_controller")
  {
    ...
    current_pos_ = Eigen::Vector3d(
        ...
        declare_parameter<double>("init_z", 0.28));
...
private:
                    ...
                    const Eigen::Vector3d &vel, double yaw, double yaw_rate)
  {
    ...
    // 高度参考: 轨迹 z 为 base_link 世界高度, 换算成五连杆腿高 (HAVEN)
    std_msgs::msg::Float32 height_msg;
    height_msg.data = static_cast<float>(
        std::clamp(pos.z() - base_z_offset_, leg_height_min_, leg_height_max_));
    leg_height_pub_->publish(height_msg);
  }
  ...
  double leg_height_min_{0.13}, leg_height_max_{0.37};
