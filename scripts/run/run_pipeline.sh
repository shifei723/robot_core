...
# install base 就是 install/ros_ws，setup.bash 在其根目录（无第二层 install/）
[ -f "$ROS_WS/setup.bash" ] && source "$ROS_WS/setup.bash"
if [ ! -d "$AGENT_WS/lib/omni_node" ]; then
    echo "[错误] $AGENT_WS/lib/omni_node 不存在，请先编译 agent_ws 工作区!"
    echo "       cd $ROBOT_CORE && ./build.sh agent_ws"
    exit 1
fi
...
# ── 3. TTS 桥接（ROS话题 -> 对应后端）──
python3 "$AGENT_WS/lib/omni_node/tts_bridge.py" \
...
# ── 4. 相机发布节点 ──
python3 "$AGENT_WS/lib/omni_node/camera_pub.py" \
    > "$LOG_DIR/camera.log" 2>&1 &
echo "camera $!" >> "$PIDFILE"
echo "[4/6] camera_pub 已启动"

# ── 5. KWS 桥接（监听状态文件下降沿，注意传入正确的 commands 路径）──
python3 "$AGENT_WS/lib/omni_node/transformVL.py" \
...
python3 "$AGENT_WS/lib/omni_node/tool_agent.py" \
...
echo "   python3 $AGENT_WS/lib/omni_node/watch_omni.py   # 推理结果实时查看"
echo "   $0 stop                            # 停止全部"
echo "============================================"
