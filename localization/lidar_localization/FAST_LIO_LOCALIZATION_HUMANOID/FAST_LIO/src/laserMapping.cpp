// This is an optimized version of the Fast-LIO algorithm to reduce pose and angle delays
// Optimizations include:
// 1. Improved time synchronization
// 2. Reduced filtering delay
// 3. Enhanced state prediction
// 4. Better IMU integration
// 5. [Architecture Fix] Asynchronous thread processing with Condition Variables
// 6. [Architecture Fix] Expanded IMU queue for high-frequency data

// Modifier: Optimized by AI Assistant
#include <omp.h>
#include <mutex>
#include <math.h>
#include <thread>
...
deque<double> state_time_buffer;
...
private:
    ...
    void process_loop()
    {
        while (rclcpp::ok())
        {
            ...
            // 等待直到有退出信号，或者IMU和LiDAR缓冲区都不为空
            sig_buffer.wait(lock, [this]() { 
                return flg_exit || (!lidar_buffer.empty() && !imu_buffer.empty()); 
            });

            if (flg_exit) break;

            // 在持有锁的情况下同步数据包
            bool sync_success = sync_packages(Measures);
            
            // 数据包同步完成后立即解锁！允许ROS回调继续接收高频传感器数据，避免阻塞
            lock.unlock(); 
            ...
            if (sync_success)
            {
                if (flg_first_scan)
                {
                    ...
                    continue; // 必须 continue 以便进入下一次循环
                }
                ...
                /*** Store current state for potential future correction ***/
                state_buffer.push_back(state_point);
                state_time_buffer.push_back(Measures.lidar_beg_time);
                
                // Keep only recent states (last 10 seconds worth)
                while (!state_time_buffer.empty() && 
                       (Measures.lidar_beg_time - state_time_buffer.front() > 10.0)) {
                    state_buffer.pop_front();
                    state_time_buffer.pop_front();
                }
