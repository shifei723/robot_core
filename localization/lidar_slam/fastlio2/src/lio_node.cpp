...
public:
    LIONode() : Node("lio_node")
    {
        ...
        m_lidar_sub = this->create_subscription<livox_interfaces::msg::CustomMsg>(m_node_config.lidar_topic, 10, std::bind(&LIONode::lidarCB, this, std::placeholders::_1));
        ...
        m_save_map_service = this->create_service<std_srvs::srv::Trigger>(
            "save_map",
            std::bind(&LIONode::saveMapCallback, this, std::placeholders::_1, std::placeholders::_2)
        );
        RCLCPP_INFO(this->get_logger(), "Save map service: /save_map");
        RCLCPP_INFO(this->get_logger(), "Save path: %s", m_node_config.save_path.c_str());
    }
    ~LIONode()
    {
        // 退出时自动保存地图
    ...
    bool saveGlobalMap()
    {
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "Map builder not ready!");
            return false;
        }

        // 1. 通过接口获取 ikdtree 的 shared_ptr
        auto ikdtree_ptr = m_builder->lidar_processor()->ikdtree();

        // 2. 检查指针和根节点
        if (!ikdtree_ptr || ikdtree_ptr->Root_Node == nullptr)
        {
            RCLCPP_ERROR(this->get_logger(), "KD-Tree is empty, no map to save!");
            return false;
        }

        RCLCPP_INFO(this->get_logger(), "Flattening ikd-tree, please wait...");

        // 3. 提取所有点云
        PointVec map_points;
        // 注意：使用 -> 调用 flatten，且 Root_Node 也是通过指针访问
        ikdtree_ptr->flatten(ikdtree_ptr->Root_Node, map_points, NOT_RECORD);

        if (map_points.empty())
        {
            RCLCPP_ERROR(this->get_logger(), "No points in map after flattening!");
            return false;
        }

        // 4. 转换并保存
        CloudType::Ptr cloud(new CloudType());
        cloud->points = map_points;
        cloud->width = cloud->points.size();
        cloud->height = 1;
        cloud->is_dense = true;

        if (pcl::io::savePCDFileBinary(m_node_config.save_path, *cloud) == 0)
        {
            RCLCPP_INFO(this->get_logger(), "*************************************");
            RCLCPP_INFO(this->get_logger(), "SUCCESS: Map saved to %s", m_node_config.save_path.c_str());
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "Failed to write PCD file to: %s", m_node_config.save_path.c_str());
            return false;
        }
    }
    ...
    void imuCB(const sensor_msgs::msg::Imu::SharedPtr msg)
    {
        ...
        //这里根据IMU的数据来看，我这里面是9.805，如果是0.9805则乘10
    ...
    void lidarCB(const livox_interfaces::msg::CustomMsg::SharedPtr msg)
    {
...
private:
    ...
    rclcpp::Subscription<livox_interfaces::msg::CustomMsg>::SharedPtr m_lidar_sub;
