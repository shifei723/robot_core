...
# 基于官方 rtabmap_examples/realsense_d435i_color.launch.py 修改：
#  本机 D435i 的 IMU 走 HID→IIO 路径，librealsense 需要写
#  /sys/bus/iio/devices/iio:deviceN/{trigger/current_trigger,buffer/*}，
#  而这些 sysfs 文件默认 root:root 0644，普通用户写不了，
#  所以 /camera/imu 不会有数据。这里去掉对 IMU 的依赖：
#   - wait_imu_to_init 改为 False（否则里程计永远等不到 IMU 不初始化）
#   - 移除 imu_filter_madgwick 节点
# 其余参数与官方保持一致，作为参照配置。
