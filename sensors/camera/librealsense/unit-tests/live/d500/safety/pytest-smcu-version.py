...
# Test to read SMCU Version from rsutils FW version, HWMC Commands and rs-fw-update tool,
# Compare against 0.0.0.0 and check if all the methods provide same result
def test_smcu_version_consistent_across_methods(test_device):
    versions_dict = {}
    ...
    log.debug("SMCU Version Using rs_camera_info: %s", fw_version)
    ...
    # log.debug ("HMC Command GVD " + str(gvd_data))
    substring = gvd_data[275:279]
    hmc_smcu_version = ".".join(str(char) for char in substring)
    log.debug("SMCU Version Using HMC: %s", hmc_smcu_version)
    ...
    log.debug("rs-fw-update path in the directory : %s", rs_fw_update_tool_path)

    version = subprocess.check_output(rs_fw_update_tool_path).decode("utf-8")

    rs_fw_update_smcu_version = re.search(r'SMCU firmware version: ([\d.]+)', version)
    ...
    log.debug("SMCU Version From one of the method = %s", str(test_val))
    for key, value in versions_dict.items():
        log.info("%s: %s", key, value)
        # check the smcu version is not zero
        check.is_true(value != "0.0.0.0", f"SMCU Version is 0.0.0.0! ({key})")
        # Check the smcu version is same across multiple methods
        check.is_true(value == test_val, f"SMCU Version is different than expected! ({key}={value}, expected={test_val})")
