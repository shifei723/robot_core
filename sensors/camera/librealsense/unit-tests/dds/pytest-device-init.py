...
#############################################################################################
#
def test_one_stream(remote_and_participant):
    ...
    for stream in device.streams():
        ...
        check.equal( stream.name(), "s1" )
        check.equal( stream.sensor_name(), "sensor" )
        check.equal( len( profiles ), 1 )
        check.equal( str(profiles[0]), "<pyrealdds.depth_stream_profile 's1' 10x10 rgb8 @ 9 Hz>" )
        check.equal( profiles[0].stream(), stream )
        check.equal( stream.default_profile_index(), 0 )
    remote.run( 'close_server()' )
    device = None
#
#############################################################################################
#
def test_motion_stream(remote_and_participant):
    remote, participant = remote_and_participant
    ...
    for stream in device.streams():
        ...
        check.equal( stream.sensor_name(), "sensor2" )
        check.equal( stream.type_string(), "motion" )
        check.equal( len( profiles ), 1 )
        check.equal( str(profiles[0]), "<pyrealdds.motion_stream_profile 's2' @ 30 Hz>" )
        check.equal( profiles[0].stream(), stream )
        check.equal( stream.default_profile_index(), 0 )
    remote.run( 'close_server()' )
    device = None
...
#############################################################################################
#
def test_no_streams(remote_and_participant):
    remote, participant = remote_and_participant
    remote.run( 'test_no_streams()' )
    device = dds.device( participant, info )
    device.wait_until_ready()  # If no device is available before timeout, this will throw
    check.is_true( device.is_ready() )
    check.equal( device.n_streams(), 0 )
    for stream in device.streams():
...
#############################################################################################
#
def test_no_profiles_should_fail_on_server_side(remote_and_participant):
    remote, _ = remote_and_participant
    try:
        remote.run( 'test_no_profiles()' )
    except test.remote.Error as e:
        # this fails because streams require at least one profile
        check.equal( str(e), "[remote] RuntimeError: at least one profile is required to initialize stream 's1'" )
    device = None
#
#############################################################################################
#
def test_n_profiles_10(remote_and_participant):
    remote, participant = remote_and_participant
    ...
    for i in range(10):
        ...
        fibo[-2] = fibo[-1]
        fibo[-1] = v
        check.equal( profiles[i].width(), i )
        check.equal( profiles[i].height(), v )
    remote.run( 'close_server()' )
    device = None
#
#############################################################################################
#
def test_d435i(remote_and_participant):
    remote, participant = remote_and_participant
    remote.run( 'test_d435i()' )
    device = dds.device( participant, d435i.device_info )
    device.wait_until_ready()  # If no device is available before timeout, this will throw
    check.is_true( device.is_ready() )
    check.equal( device.n_streams(), 5 )
    for stream in device.streams():
        profiles = stream.profiles()
    remote.run( 'close_server()' )
    device = None
...
#############################################################################################
#
def test_discovery_of_another_client_device(remote_and_participant):
    remote, participant = remote_and_participant
    remote.run( 'test_one_stream()' )
    device = dds.device( participant, info )
    device.wait_until_ready()  # If no device is available before timeout, this will throw
...
#############################################################################################
#
def test_multiple_overlapping_client_initializations(remote_and_participant):
    remote, _ = remote_and_participant
    remote.run( 'test_one_stream()' )
